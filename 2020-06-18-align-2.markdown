...
> To activate all of the constraints at the same time, wrap the code that creates the constraints into `Constraints {}` closure. Internally, it will keep track of all of the constraints that you create, and when you are done, it will use [`NSLayoutConstraints.activate()`](https://developer.apple.com/documentation/uikit/nslayoutconstraint/1526955-activate) to activate them all at the same time, which is faster than doing it one by one.
{:.info}
