...
<img class="Screenshot" src="{{ site.url }}/images/posts/vpn-protocol/vpn-01.png">
</div>

<div id="page-02" class="SwiftUITabContent SwiftUIExampleWithScreenshot2 Any-responsiveCard SwiftUIPost_ExampleWithTabs" markdown="1" style="display:none">
The DNS server returns an *answer* – `93.184.216.34`.
<img class="Screenshot" src="{{ site.url }}/images/posts/vpn-protocol/vpn-02.png">
</div>
</div>
...
<img class="Screenshot" src="{{ site.url }}/images/posts/vpn-protocol/vpn-03.png">
</div>
...
<img class="Screenshot" src="{{ site.url }}/images/posts/vpn-protocol/vpn-04.png">
</div>
</div>
...
> On Apple platforms, there is a variety of APIs for working with ports/sockets, ranging from [`CFSocket`](https://developer.apple.com/documentation/corefoundation/cfsocket-rg7) to recently introduced [`Network` framework](https://developer.apple.com/documentation/network/nwconnection).
{:.info}
...
<img class="Screenshot Any-responsiveCard" src="{{ site.url }}/images/posts/vpn-protocol/vpn-05.png">
...
Now, to know which computers on the network to send the messages to, the TCP/UDP are wrapped yet again, this time using IP protocol. Let's get back to our Wireshark capture to see what information an IP header typically contains.

<img class="Screenshot Any-responsiveCard" src="{{ site.url }}/images/posts/vpn-protocol/vpn-06.png">

All of this information that you saw in the [Wireshark](https://www.wireshark.org) captures recorded from my `en0` *network inteface* (WiFi), is also visible to my [ISP](https://en.wikipedia.org/wiki/Internet_service_provider) and to the services to which the requests are sent. They know my IP address, they know which port I'm using, etc. Using this information they can identify and track me. And both HTTP and DNS protocols do not have ny encryption, so anyone in between my computer and the destintion can read all of my communications. And this is a problem that VPN is designed to solve.
...
>
> There are *physical* network interfaces, such as `en0` (which is your WiFi module on macOS). You can list all of those using `networksetup -listallhardwareports` command. Each physical inteface has an associated Ethernet Address and a hardware device, for example:
>
...
What is VPN? Originally, VPN (Virtual Private Network) was designed to establish secure connections between a computer and a protected or private corporate network, using an otherwise insecure public network (such as the Internet). This typically involved "making" a computer part of the private network so that the computers within the network could address it. 

In recent years, VPNs also blown up in popularity in the consumer market as a way to stay secure and anonymous when going online, and also enjoying unrestricted network access by hiding the source of traffic. I'm going to focus primarily on this use case as it typically involves a bit less complexity.
...
> - Encrypt them
> - Encapsulated encrypted items in the VPN packets (specific to the VPN protocol)
> - Send the VPN packets to the VPN server using UDP, TCP, or other standard transport layer protocol

The IP packets end up being wrapped into another IP packets, a networking version of a Russian nesting doll.

<img class="Screenshot Any-responsiveCard" src="{{ site.url }}/images/posts/vpn-protocol/vpn-matryoshka.png" style="width: 240px; border-radius: 44px;">

This is a simple idea, yet it achieves all of our goals. The original IP packets can be fully encrypted. You can't see neither application, nor even Internet layer contents. The only thing ISP or other observers are going to see are communication between a VPN client and a server.

Now, how do you implement this? This is what I'm going to cover in the upcoming articles. But on a high level, it is going to involve directly manipulating IP packets, encryption, virtual network interfaces, and more.

### Example: OpenVPN

Before designing our own VPN protocol in the upcoming articles, it is worth exploring an existing VPN protocol, such as [OpenVPN](https://openvpn.net).

The [description](https://openvpn.net/community-resources/openvpn-protocol/) of the protocol is surprisingly simple. First, the client **establishes the session** with the server and exchanges cryptographic information.
...
<img class="Screenshot" src="{{ site.url }}/images/posts/vpn-protocol/openvpn-01.png">
</div>
...
The server responds with a control packet with `P_CONTROL_HARD_RESET_SERVER_V2` type.
<img class="Screenshot" src="{{ site.url }}/images/posts/vpn-protocol/openvpn-02.png">
</div>
...
The client acknowledges that it received the packets form "reset" type from the server.
<img class="Screenshot" src="{{ site.url }}/images/posts/vpn-protocol/openvpn-03.png">
</div>
</div>

When the session is ready, the client and the server exchange cryptographic information using TLS protocol.
...
The client sends another control (`P_CONTROL`) packet.
<img class="Screenshot" src="{{ site.url }}/images/posts/vpn-protocol/openvpn-04.png">
</div>
...
<img class="Screenshot" src="{{ site.url }}/images/posts/vpn-protocol/openvpn-05.png">
</div>
</div>

I'm not going to go into detail regarding TLS exchange. The point is that after a few control messages, the client becomes ready to encrypt and send messages to the server. And this is what `P_DATA_V1` message type is for – data channel packet containing actual tunnel data ciphertext.

<img class="Screenshot Any-responsiveCard" src="{{ site.url }}/images/posts/vpn-protocol/openvpn-06.png">

And this wraps up an overview of OpenVPN. Naturally, it's a simplified view, there are more options that OpenVPN offer. But the main takeaways are:
...
- The messages are send over UDP[^2]. The control messages (`P_CONTROL_V1`) have a delivery guarantee and require acknowledgment (`P_ACK_V1`). The data messages (`P_DATA_V1`) don't require an acknowledgment and don't have a delivery guarantees, the underlying protocol of the original messages is responsible for providing the needed guarantees.

With this in mind, we are ready to move forward.
...
5. Wireshark, [**OpenVPN Sample Captures**](https://wiki.wireshark.org/OpenVPN)
6. OpenVPN, [**OpenVPN Protocol**](https://openvpn.net/community-resources/openvpn-protocol/)
7. Cloudflare, [**What is DNS?**](https://www.cloudflare.com/learning/dns/what-is-dns/)
8. Mozilla, [**HTTP**](https://developer.mozilla.org/en-US/docs/Web/HTTP) 
...
[^3]: HTTP/3 seems to be [replacing TCP with UDP](https://thenewstack.io/http-3-replaces-tcp-with-udp-to-boost-network-speed-reliability/)
