...
description: A Russian nesting doll of network protocols
...
When the session is ready, the client and the server exchange cryptographic information using TLS protocol[^7].
...
I'm not going to go into detail regarding the TLS exchange. The point is that after a few control messages, the client becomes ready to encrypt and send messages to the server. And this is what `P_DATA_V1` message type is for – data channel packet containing actual tunnel data ciphertext.
