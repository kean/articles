...
I've been using SwiftUI a lot lately to build [Pulse](https://github.com/kean/Pulse) and loved many of the ideas behind it. So I was thinking, can we apply these ideas to the apps written with RxSwift or ReactiveSwift? SwiftUI also uses a reactive framework, Combine, for updating its views. But as a user, you don't even think about Combine when writing SwiftUI code. Why should it be different with RxSwift?

I already spent a significant amount of time [investigating](https://kean.blog/post/swiftui-data-flow) how Swift Data Flow works under the hood last year, so I had a good idea of what I needed to do to make it work. I built a quick proof-of-concept project called [RxUI](https://github.com/kean/RxUI) to demonstrate this different way of using RxSwift. It especially suits folk who prefer to use unidirectional view models.
