...
layout: post
title: "Nuke Docs"
subtitle: Building new documentation for Nuke
description: Building new documentation for Nuke
...
Speaking about search; it was probably the most exciting feature I needed to build. It's hard to find good articles on a subject, so maybe someone will find it helpful. 
...
The search is built using [lunr.js](https://lunrjs.com) which is a simple client-side search engine. It doesn't provide any of the UI components; you have to build them yourself. But first, you need to create a search index.

In Nuke Docs, an index (`search-index.json`) is a JSON file that gets generated as part of the Jekyll build process. The file has [front matter](https://jekyllrb.com/docs/front-matter/) to enable special processing where you can use [Liquid](https://jekyllrb.com/docs/front-matter/) tags and have access to all site pages and other information.
...
As an output, you get a simple `search-index.json` file with an array of entries ([example](https://kean.blog/nuke/assets/search-index.json)). When the user presses the search button (or `f` key), the app fetches this file, and populates the lunr.js index.

```javascript
function populateSearchIndex(json) {
    let entries = json.flatMap(splitPageContentIntoChunks);
...
Starting with lunr.js 2.0, by default, the search simply looks for a complete match of the input word. For example, if you search for `priorirty`, the text has to contain a complete word. If you just enter `priori`, the results will be empty. So I ended up using the following approach for creating a query:

```javascript
const updatedQuery = query
    .split(" ")
    .map(word => word + '~1^2 ' + word + '*')
    .join(' ');

const results = searchIndex.search(updatedQuery);
```

I search for a full match of the word with a bit of fuzziness (`~1`) and boost the score for this match (`^2`), and I also search for a prefix using a wildcard (`*`). There is probably a better way to configure it, but with this setup, I got it to work as the previous versions.

Now, this search works, but it only searches complete pages. And I wanted to allow the user to search for parts of the guide split by headings, and jump straight to the heading. Fortunately, I already had [jekyll-anchor-headings](https://github.com/allejo/jekyll-anchor-headings) set up for the table of contents, so all I needed to do was a bit of pre-processing of the search index.

I take each page, split it into chunks using simple regex[^5] and voila. I then strip HTML and liquid tags from the content, and it's done. The only thing left is to create an anchor link for each entry and attach it to our new piece of searchable content.

I don't want to paste the complete code here, but you can always find it on the [website](https://kean.blog/nuke/guides/welcome). As for the UI, you can also find it there. There is quite a lot of JavaScript with event handlers to make it work well, especially to support keyboard navigation.

[^5]: `.content.split(/(<h[^>]*>.*?<\/h.>)/g)`
